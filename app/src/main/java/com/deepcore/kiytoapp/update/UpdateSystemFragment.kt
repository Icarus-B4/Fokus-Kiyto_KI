...
class UpdateSystemFragment : Fragment() {
    ...
    private fun showUpdateStatus() {
        // Hole Informationen vom UpdateManager
        ...
        // Vergleiche die Versionen, um zu wissen, ob lokale Version neuer ist
        val versionCompareResult = if (latestVersion != getString(R.string.error_checking_updates)) {
            updateManager.compareVersions(latestVersion, currentVersion)
        } else {
            0
        }

        LogUtils.debug(this, "Update Status: currentVersion=$currentVersion, latestVersion=$latestVersion, updateAvailable=$updateAvailable, versionCompareResult=$versionCompareResult")
    ...
    private fun showUpdateDialog() {
        updateManager.updateDescription?.let { description ->
            // Verwende die Download-URL, wenn verfügbar, sonst die Update-URL
            val downloadUrl = updateManager.downloadUrl ?: updateManager.updateUrl
            downloadUrl?.let { url ->
    ...
    private fun showDebugLogs() {
        val logs = StringBuilder()
            ...
            .append("Download URL: ${updateManager.downloadUrl}\n")
            .toString()
            
        // Zeige Dialog mit Debug-Logs und Test-Button
        val dialog = DebugLogDialog.newInstance(logs)
        dialog.setTestModeListener {
            // Erzwinge Update-Prüfung im Test-Modus
            forceUpdateCheck()
        }
        dialog.show(parentFragmentManager, "debug_logs")
    }
        ...
        lifecycleScope.launch {
            try {
                ...
                if (updateAvailable) {
                    ...
                    // Zeige Toast, wenn kein Update verfügbar ist
                    Toast.makeText(requireContext(), "Kein Update verfügbar", Toast.LENGTH_SHORT).show()
                }
                ...
                LogUtils.error(this@UpdateSystemFragment, "Fehler beim Update-Check", e)
                updateAnimationState(isError = true)
            }
        }
    }
